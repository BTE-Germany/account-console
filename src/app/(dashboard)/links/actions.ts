...
import { syncDiscordSubscription, syncMinecraftSubscription, syncUserSubscription } from "@/lib/rankSync";
...
export async function unlinkAccount(identityProvider: string) {
    ...
    if (identityProvider === "discord") {
        await syncDiscordSubscription(user.federatedIdentities.find(identity => identity.identityProvider === "discord")?.userId || "", false)
    }
...
export async function unlinkMinecraftAccount() {
    ...
    await syncMinecraftSubscription(user.attributes.minecraft_uuid[0], false)
    return true
}
